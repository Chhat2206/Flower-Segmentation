    # https://www.researchgate.net/publication/368908226_Comparative_Analysis_of_Image_on_Several_Edge_Detection_Techniques
    ...
    def apply_prewitt_edge_detection(image):
        # Define the Prewitt kernels in x and y direction
    ...
    def process_and_compare_image(input_path, ground_truth_path):
        ...
        # Apply Prewitt Edge Detection here
        prewitt_edge_image = apply_prewitt_edge_detection(noise_reduced_image)
        additionally_blurred_image = apply_additional_blur(prewitt_edge_image)
        ...
        images = [image, gray_image, noise_reduced_image, prewitt_edge_image, additionally_blurred_image, binary_image,
                  binary_ground_truth, morphologically_transformed_image]
        descriptions = [
            "Original Image",
            "Grayscale Conversion",
            "Noise Reduction (Gaussian Blur)",
            "Prewitt Edge Detection",  # Updated description
            "Additional Blur",
    ...
    def process_images_and_compare(directory_paths, ground_truth_directory_paths):
        ssim_scores = {}
        total_images = 9  # Adjust as needed based on actual total images
        steps_per_set = 8  # Adjusted to include Prewitt edge detection in descriptions
        ...
        for difficulty in ['easy', 'medium', 'hard']:
            for i in range(1, 4):  # Assuming there are 3 images per difficulty level, adjust as needed
                ...
                try:
                    ...
                    for step_index, (img, desc) in enumerate(zip(images, descriptions)):
                        if step_index >= axs.shape[1]:  # Check to prevent index out of range if more steps than expected
                            break
                        ax = axs[current_image_index % total_images, step_index]  # Adjust for total_images
