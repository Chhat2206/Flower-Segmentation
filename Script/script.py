...
def process_and_compare_image(input_path, ground_truth_path):
    ...
    # Prewitt Edge Detection is applied directly after noise reduction
    prewitt_edge_image = apply_prewitt_edge_detection(noise_reduced_image)

    # Since we are removing the additional blurring, we directly threshold the Prewitt edge image
    binary_image = threshold_image(prewitt_edge_image)

    # Morphological transformation is applied to the binary image
    ...
    images = [image, gray_image, noise_reduced_image, prewitt_edge_image, binary_image, binary_ground_truth,
